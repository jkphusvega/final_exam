...
그대로 임포트하면 '재수강' 컬럼을 제외한 모든 컬럼의 타입이 text로 설정되어 2,455개의 행이 임포트된다.
(공지에 따르면 2,455개로 임포트해도 무방하나, 사이드 메뉴와 분석 데이터 결과가 달라질 수 있다.)

본 프로젝트에서는 데이터 정확도를 위해 아래와 같이 타입을 지정하여 임포트하였다:
- '순번' 컬럼을 Primary Key로 설정
- '순번', '학점', '시수', '이론', '실습', '정원', '수강', '수강(남)', '수강(여)', '재수강' 컬럼 타입을 int8으로 변경

임포트 후 SQL Editor에서 아래 쿼리로 총 행 수를 확인:

```sql
select count(*) from courses;
```

결과값이 2,313인지 확인. (타입 변환 과정에서 오류가 있는 행이 제외되어 2,455개에서 2,313개로 줄어든 것이 정상)
...
> 헤더 INU Blue 배경 / 수강률 기준 Badge 색상 강조(90% 이상 초록, 75-89% 파랑, 50% 미만 빨강)
>
...
이 대시보드의 데이터는 **2026학년도 1학기 시간표**로, 학기 중 변경이 거의 없는 고정값이다.
실시간으로 갱신되어야 하는 데이터가 아니기 때문에, 매번 서버에 재요청하는 것은 불필요한 비용이다.

이 특성을 활용하여 앱 시작 시 전체 데이터(2,313개)를 한 번만 fetch하고 브라우저 메모리에 캐싱하였다.
이후 사이드바 선택, 검색, 차트 클릭 등 모든 인터랙션은 네트워크 호출 없이
클라이언트에서 즉시 계산하여 반응 속도를 0ms에 가깝게 구현하였다.

데이터가 자주 바뀌는 서비스라면 적합하지 않은 방식이지만,
학기 단위로 고정되는 강좌 데이터의 특성상 최적의 아키텍처 선택이라 판단하였다.
